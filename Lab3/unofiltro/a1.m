...
b_vals = [1.5, 1.5, 1.5, 5.0];
v0_vals = [2.0, 0.3, 20.0, 2.0];
case_names = {'good', 'low_v0', 'high_v0', 'high_noise'};
...
for i = 1:length(b_vals)
    ...
    exportgraphics(fig_t, sprintf('img/lpf_time_%s.pdf', case_names{i}), 'ContentType', 'vector');
    close(fig_t);
    ...
    exportgraphics(fig_f, sprintf('img/lpf_spec_%s.pdf', case_names{i}), 'ContentType', 'vector');
    close(fig_f);
end